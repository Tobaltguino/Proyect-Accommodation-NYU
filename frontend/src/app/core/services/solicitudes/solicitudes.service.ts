...
import { SolicitudDTO, CambiarEstadoSolicitudRequest } from '../../../shared/models';
...
export class SolicitudesService {
  ...
  obtenerTodasLasSolicitudes(): Observable<SolicitudDTO[]> {
    return this.http.get<SolicitudDTO[]>(`${this.apiUrl}/solicitudes-admin/todas`, {
  ...
  cambiarEstadoSolicitud(idSolicitud: number, nuevoEstado: string): Observable<SolicitudDTO> {
    ...
    return this.http.patch<SolicitudDTO>(
      `${this.apiUrl}/solicitudes-admin/idSolicitud/${idSolicitud}`, 
      payload, 
      { headers: this.getAuthHeaders() }
    );
  }
