...
export class HabitacionesService {
  ...
  async modificarHabitacion(
    idHabitacion: number,
    datosActualizados: Partial<HabitacionEntity>,
  ) {
    ...
    if (!habitacion) {
      throw new NotFoundException(
        `La habitación con ID ${idHabitacion} no existe.`,
      );
    }
  ...
  async obtenerPorEdificio(idEdificio: number): Promise<HabitacionEntity[]> {
    return await this.habitacionRepo
  ...
  async obtenerTotalDisponibles(): Promise<{ total: number }> {
    const cantidad = await this.habitacionRepo
      .createQueryBuilder('habitacion')
